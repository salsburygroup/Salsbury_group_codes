...
def calculate_rmsf(structure_file, xtc_file, output_name, selection='all', selection_name=None):
    ...
    n_frames = min(1000, len(u.trajectory))  # Ensure not to exceed the total number of frames

    # Calculate the average coordinates over the first 2000 frames
    for ts in u.trajectory:
      if u.trajectory.frame >= n_frames:
        break
      average_coordinates += atoms.positions
    average_coordinates /= n_frames

# Reset the trajectory iterator
    u.trajectory[0]

# Calculate the rmsf over the first 2000 frames
    for ts in u.trajectory:
     if u.trajectory.frame >= n_frames:
        break
